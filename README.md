...
**1단계 전처리 + 2단계 Rule 검출기 구현 완료.** kNN 유사도 검색과 대시보드는 아직 없다.
...
npm run dev      # 개발 서버
npm test         # 99개 테스트
npm run build    # dist/index.html 단일 파일 생성 (~41KB)
```
...
3. 라벨링 UI 및 학습 데이터 관리 (IndexedDB)
4. kNN 유사도 검색 및 융합 판정
5. 대시보드 — 불량 비율, 위치 상관성, Lot/설비/공정 분석

현재 검증은 모두 합성 이미지 기준이다. 임계값(`src/core/settings.ts`)은 추정치이며,
실제 점등 이미지로 튜닝해야 한다.
