...
```text
black_signal = max(R, G, B)   # 세 채널이 모두 내려가야 낮아짐
white_signal = min(R, G, B)   # 세 채널이 모두 올라가야 높아짐
black_bg     = fitBackgroundSurface(black_signal, activeMask)
white_bg     = fitBackgroundSurface(white_signal, activeMask, bright_polarity)
dark_residual   = int16(black_signal) - int16(black_bg)
bright_residual = int16(white_signal) - int16(white_bg)
```

암점·암선은 실제로 Black으로만, 명점·명선은 실제로 White로만 발현된다는 현장 조건을 직접 사용한다.
예를 들어 R 패턴의 정상 화소가 `[190, 14, 14]`이고 White 불량이 `[190, 190, 190]`이면 R 채널 자체는
변하지 않는다. 기존 R 채널 단독 방식은 이를 놓치지만 `min(R,G,B)`는 `14 → 190` 상승을 검출한다.
암부와 명부는 서로 다른 신호와 배경 곡면을 사용하므로 한쪽 불량이 다른 쪽 배경 추정을 왜곡하지 않는다.
...
1. 히스토그램이 실제로 이봉(bimodal)일 때만 **Otsu 분할**로 불량 쪽 클래스를 사전 배제한다.
   보통은 낮은 휘도 클래스를 암부 불량으로 제외한다. 반대로 이미 점등된 낮은 클래스 위에 작은 비율의
   높은 휘도 클래스가 있으면 이를 두꺼운 명점·명선 영역으로 판단해 높은 클래스를 제외한다.
   이 방향 판별이 없으면 두꺼운 명선을 배경 자체로 피팅해 residual이 0으로 사라질 수 있다.
2. **Tukey biweight IRLS**로 남은 이상치(주로 가는 명점·명선)를 억제한다.

따라서 가는 명선뿐 아니라 Active 영역의 유의한 면적을 차지하는 두꺼운 명선도 배경 모델에 흡수되지 않는다.

### 3.2 암부 / 명부 후보

```text
dark_mask   = dark_residual < -dark.residual_threshold
bright_mask = bright_residual > +(W ? bright.w_residual_threshold
                                    : bright.residual_threshold)
```

W는 정상 배경도 White라 RGB보다 남는 대비가 작으므로 별도 임계값(기본 20)을 쓴다. W Active 영역에서
`min(R,G,B) >= 250`인 픽셀이 1% 이상이면 센서/영상 포화로 더 밝은 불량의 대비가 사라질 수 있다고 보고
품질 경고와 `검수 필요`를 표시한다.
...
다음 중 하나면 **Line 형상**으로 인정한다.

- 일반 Line: `aspect >= line.min_aspect_ratio` 이고 `length_ratio >= line.min_length_ratio`
- 두꺼운 Line: `aspect >= line.thick_min_aspect_ratio` 이고
  `length_ratio >= 1.5 × line.min_length_ratio`

두꺼운 Line 분기는 충분히 긴 성분에만 적용해 원형 명점이나 짧은 사각 얼룩이 Line으로 바뀌는 것을 막는다.
...
```text
mean_luma = black_signal[active_mask].mean()   # 필드명은 이전 버전 호환을 위해 유지
...
2. 같은 위치에서 암점용 설정 개수 이상의 패턴으로 확인된 암점만 남긴다.
   dark_area_ratio = max(패턴별 confirmed dark_mask 면적 / active_area_px * 100)
   0 < ratio <= 5   → 암점 小
   5 < ratio <= 15  → 암점 中
   ratio > 15       → 암점 大
3. 명점/명선/암선은 defect-family별 설정 개수 이상의 패턴에서 검출될 때 인정한다.
   중심 거리, 면적 유사도, bbox IoU를 확인하며 Line은 주축 방향 차이도 검사한다.
...
2~3번의 교차 규칙은 서로 다른 위치의 먼지·노이즈가 하나로 합쳐지는 오검출을 줄인다. 특정 색에서만
나타나는 서브픽셀 불량은 명점/Line 확인 수를 1로 둘 수 있다. 앱은 이 후보를 버리지 않고 판정에 유지하되
`PATTERN_ONLY_DEFECT` 검수 사유와 Sorting `HOLD`를 붙인다.
...
Black/White 물리 신호 마스크를 사용하는 `feature_version = "v2"` 로 태깅한다. 앱 시작 시 이미지가 남아
있는 승인 학습 패널의 구버전 벡터는 자동 재생성한다.

### 5.2 검색

```text
후보 = embedding WHERE is_searchable = 1 AND feature_version = 'v2'
