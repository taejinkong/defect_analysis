...
bg       = medianFilter(gray, radius = 25)   # 조명 불균일 제거
residual = int16(gray) - int16(bg)
```

배경 추정에 **평균이 아니라 중앙값**을 쓴다. 평균은 불량 자체에 끌려가서 잔차를 깎아먹는다.
넓은 암점 위에서는 배경 추정치가 함께 낮아져 불량이 사라진다.

51×51 중앙값 필터를 소박하게 구현하면 512×512 이미지에서 수 초가 걸린다.
**Huang의 상수시간 슬라이딩 히스토그램 알고리즘**을 쓴다. 256-bin 히스토그램을 유지하며 창을 한 칸씩
옮길 때 열 단위로만 갱신하므로 커널 크기와 무관하게 픽셀당 O(1)이다. 512×512 기준 10ms 수준이다.

Active 마스크 밖은 전부 0으로 만든다. 마스크 경계에서 중앙값 창이 배경을 포함하지 않도록,
창 안의 마스크 밖 픽셀은 히스토그램에서 제외한다.
...
각각 3×3 침식 후 팽창(morphological open)으로 노이즈를 제거하고, 4-이웃 연결 성분 분석 후
`blob.min_area_px` 미만의 성분을 버린다.
