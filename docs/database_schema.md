...
- line/box의 화면 복원 시 첫 점은 대표점과 끝점으로 `start = 2 * (x, y) - (x2, y2)` 계산한다.
  따라서 위치 분석용 대표점을 유지하면서 사용자가 드래그한 전체 선·박스를 정확히 다시 그릴 수 있다.
- 수동 암점 라벨은 UI에서 `암점 영역` 하나로 입력한다. 같은 이미지의 box 영역을 합집합으로 계산하고,
  패턴별 면적비 중 최댓값으로 D001/D002/D003 중 하나를 자동 저장한다.
...
> 현재 브라우저 구현은 IndexedDB version 4다. 기존 `panels`, `images`, `annotations`, `embeddings`를
> 유지하면서 v3에 `preprocessingResults`, `detectionResults`, `panelDecisions`, `thresholdConfigs`, v4에
> `reviews`를 비파괴적으로 추가한다. 상세 필드는 [DATA_MODEL.md](DATA_MODEL.md)를 따른다.
...
| `bright.residual_threshold` | 30 | RGB 패턴 명부 잔차 임계 |
| `bright.w_residual_threshold` | 20 | W 패턴 명부 잔차 임계 |
| `blob.min_area_px` | 6 | 최소 검출 면적 |
| `pattern.position_tolerance_r` | 0.08 | 패턴 간 동일 불량 중심 허용거리 (Active 반지름 대비) |
| `pattern.line_angle_tolerance_deg` | 15 | 패턴 간 동일 Line 주축 허용각 |
| `line.min_aspect_ratio` | 8.0 | Line 인정 종횡비 |
| `line.thick_min_aspect_ratio` | 3.5 | 충분히 긴 두꺼운 Line 인정 종횡비 |
